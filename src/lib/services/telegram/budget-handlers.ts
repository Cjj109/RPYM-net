...
export async function linkBudgetToCustomer(db: D1Database | null, budgetId: string, customerNameOrId: string | number, existingBcvRate?: { rate: number }): Promise<{ success: boolean; message: string; customerId?: number }> {
  ...
  try {
    ...
    `).bind(
      ...
      description,
      budget.total_usd,
      currencyType === 'divisas' ? 0 : budget.total_bs,
...
export async function createBudgetFromText(db: D1Database | null, text: string, mode: string, baseUrl: string, apiKey: string, adminSecret: string, hideRate: boolean = false, originalText?: string): Promise<string> {
  ...
  try {
    ...
    await db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'telegram', ?, ?)
    `).bind(
      id, fechaPresupuesto, JSON.stringify(presupuestoItems), totalUSD,
      pricingMode === 'divisa' ? 0 : totalBs,
...
export async function createCustomerPurchaseWithProducts(
  ...
  try {
    ...
    `).bind(
      ...
      description,
      totalUSD,
      currencyType === 'divisas' ? 0 : totalBs,
      pricingMode === 'dual' ? totalUSDDivisa : null,
      currencyType,
      presupuestoId,
      bcvRate.rate
